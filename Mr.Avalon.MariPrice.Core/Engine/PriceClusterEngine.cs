...
namespace Mr.Avalon.MariPrice.Core
{
	public partial class PriceClusterEngine
	{
		...
		public void Update(MariPriceApi.Price.Cluster.Update request, MariPriceDb.Price.Cluster originCluster = null)
		{
			...
			if (request.UpdationList.Contains(nameof(request.Name)) && request.Name != originCluster.Name)
				ValidateName(request.Name);

			if (request.UpdationList.Contains(nameof(request.Metal)) && request.Metal != originCluster.Metal)
				ValidateMetal(request.Metal);

			if (request.UpdationList.Contains(nameof(request.Quality)) && request.Quality != originCluster.Quality)
				ValidateQuality(request.Quality);
		...
		public List<MariPriceApi.Price.Cluster> Get(MariPriceApi.Price.Cluster.List request)
		{
			...
			foreach (var item in dbResult)
			{
				...
				{
					newItem.VariantsSettings = currentSettings.OrderBy(x => x.OrderMetalWeight).Select(x => new MariPriceApi.Price.ClusterSetting().CopyFrom(x, m_dto)).ToList();
				}
		...
		private void ValidateMetal(Guid? metalUid)
		{
			var metal = m_specEngine.GetVocValues(m_specSettings.Metal, metalUid ?? Guid.Empty).Items.SingleOrDefault();

			if (metal == null)
				throw new RecordNotFoundApiException("There is no such metal");

			if (metal.State == SpecApi.VocValueState.Deleted)
				throw new ConflictApiException("Specific metal is not available now");
		}

		private void ValidateQuality(Guid? qualityUid)
		{
			var quality = m_specEngine.GetVocValues(m_specSettings.Quality, qualityUid ?? Guid.Empty).Items.SingleOrDefault();

			if (quality == null)
				throw new RecordNotFoundApiException("There is no such quality");

			if (quality.State == SpecApi.VocValueState.Deleted)
				throw new ConflictApiException("Specific quality is not available now");
		}
	}
}
